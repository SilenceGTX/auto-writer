...
发布包内嵌对应平台的 Python 运行时（`backend/.venv/`），**用户不必单独安装 Python**。请下载与本机匹配的压缩包（如 `win-x64`），不要把 Linux/macOS 包拷到 Windows 使用。若 `start.bat` 闪退，在「命令提示符」中运行它以查看报错，并确认解压后存在 `backend\.venv\Scripts\python.exe`。
