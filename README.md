...
- **世界观设定**：按种类（人物 / 地点 / 物品 / 概念，可自定义）管理设定条目，支持键值属性，以及同种类下属性名称复用（模板候选）。
...
- **中英双语**：界面语言可在侧边栏或系统设置中切换（`zh` / `en`）；同一开关同时控制 **UI 文案与 AI prompt**（后端按 `Accept-Language` 选用 Jinja2 模板）。用户创作内容（正文、设定名、灵感等）不翻译。
- **系统设置**：多 LLM 连接配置与任务分工、全局偏好（创造力/聚焦度/篇幅档位 + 高级参数）、写作风格、界面语言、数据保存、字体与排版，以及配置一键导入 / 导出。
...
- **前端**: React 18 + TypeScript + Vite + HeroUI + Tailwind；`i18next` / `react-i18next`；pnpm 管理依赖，ESLint 检查，Vitest 测试。
- **后端**: Python 3.11 + FastAPI + SQLAlchemy（异步）+ SQLite；Jinja2 prompt 模板；uv 管理依赖，Ruff 检查，Pytest 测试，loguru 日志。
...
发布包是 **self-contained**：解释器与依赖都在 `backend/runtime/python/` 内，**用户不必安装 Python**。请下载与本机匹配的压缩包（如 `win-x64`），不要跨平台拷贝。
...
手动打包（会自动执行 ``pnpm build``，无需事先构建 ``frontend/dist``）：
...
1. 将要发布的改动 **merge 到远程 `main`**，并确认本地跟踪的是最新 `origin/main`（不要用过时的本地 `main` 打 tag）。
2. 在该提交上打 `v*` tag 并推送（例如 `git tag v0.4.0 && git push origin v0.4.0`）。
3. Release 工作流会校验 tag 落在 `main` 历史内、源码含关键模块（含 i18n / prompt 模板），再重新 `pnpm build`、组装三平台包并上传到 GitHub Releases。

也可在 Actions 中手动触发 `Release` workflow（`workflow_dispatch`），并指定版本标签。
...
- **Docker 与桌面版数据互不共享**：在 Docker 里写的作品不会自动出现在桌面安装包里，除非启动前设置 `AW_DATA_DIR` 指向同一目录（例如项目下的 `data/`）。
- 桌面版启动后访问 `http://127.0.0.1:8000/api/health`，响应中的 `version` / `git_commit` / `built_at` 可用来确认是否为最新打包版本。
...
```
├── .github/workflows/        # CI 与 Release 打包
├── backend/                  # FastAPI 后端
│   ├── app/
│   │   ├── core/             # 配置、日志、路径
│   │   ├── deps/             # FastAPI 依赖（含 locale）
│   │   ├── models/           # SQLAlchemy ORM 模型
│   │   ├── prompts/          # Jinja2 LLM prompt 模板（zh / en）
...
│   │   ├── context/          # 全局状态（主题 / 当前作品 / 辅助区 / locale）
│   │   ├── hooks/            # 复用 React hooks
│   │   ├── i18n/             # i18next 初始化
│   │   ├── locales/          # zh / en 文案（按 namespace）
│   │   ├── pages/            # 各功能页面（含 outline / writing / review 等子目录）
│   │   ├── utils/            # 纯函数工具
│   │   ├── test/             # Vitest 测试
│   │   ├── api.ts            # API 客户端（含 Accept-Language）
│   │   ├── App.tsx           # 根组件
│   │   ├── main.tsx          # 入口
│   │   └── styles.css        # 全局样式
│   ├── Dockerfile
...
│   ├── package.json
│   └── vite.config.ts
├── designs/                  # 产品与技术设计文档（见 designs/INDEX.md）
├── scripts/                  # 桌面打包与启动脚本
...
├── AGENTS.md                 # 给 coding agent 的仓库约定
├── README.md                 # 本文件（中文）
...
国际化设计说明见 [`designs/I18N.md`](designs/I18N.md)；后端模块关系见 [`designs/BACKEND_ARCHITECTURE.md`](designs/BACKEND_ARCHITECTURE.md)。
